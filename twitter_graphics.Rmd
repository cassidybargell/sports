...
merged %>% filter(sport == "rugby") %>%
        ggplot(aes(x = organization, y = prop, fill = organization)) + geom_col()

organ <- readRDS("sport-perceptions/raw-data/organ.RDS")

organ %>% 
  mutate(newprop = prop * 28)
```

```{r}
# Want to be able to display all the tweets that I am using in my data on my
# shiny app
...
football %>%
  ...
  mutate(contain = purrr::map_dfc(football, ~ str_detect(football$stripped_text, "injury")))
