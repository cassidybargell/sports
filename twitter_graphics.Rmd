...
proportion_words <- function(tib){
  tib %>%
  mutate(has = ifelse(str_detect(text, regex("injury", ignore_case = TRUE)), 1, 0)) %>%
  mutate(has = ifelse(str_detect(text, regex("injuries", ignore_case = TRUE)), 1, 0)) %>%
  mutate(has = ifelse(str_detect(text, regex("concussion", ignore_case = TRUE)), 1, 0)) %>%
  mutate(has = ifelse(str_detect(text, regex("concussions", ignore_case = TRUE)), 1, 0)) %>%
  mutate(has = ifelse(str_detect(text, regex("cte", ignore_case = TRUE)), 1, 0)) %>%
  count(has) %>%
  mutate(total = sum(n)) %>%
  filter(has == 1) %>%
...
research_prop <- proportion_words(research_tml)
```
...
foot_cw <- football %>%
  mutate(has_injury = ifelse(str_detect(text, 
                                        regex("injury", ignore_case = TRUE)), 1, 0)) %>%
  mutate(has_injuries = ifelse(str_detect(text, 
                                          regex("injuries", ignore_case = TRUE)), 1, 0)) %>%
  mutate(has_concussion = ifelse(str_detect(text, 
                                            regex("concussion", ignore_case = TRUE)), 1, 0)) %>%
  mutate(has_concussions = ifelse(str_detect(text, 
                                            regex("concussions", ignore_case = TRUE)), 1, 0)) %>%
  mutate(has_cte = ifelse(str_detect(text, 
...
research_cw <- contains_word(research_tml, "ncaaresearch")
...
tweets <- football_cw %>%
  full_join(rugby_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(hockey_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(soccer_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(basketball_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(nfl_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(nhl_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(nba_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(ncaa_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(research_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(ncaafoot_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(usarugby_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(sr_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(wr_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(ussoccer_cw, by = c("screen_name", "text", "choice")) %>%
  full_join(usahockey_cw, by = c("screen_name", "text", "choice")) 
